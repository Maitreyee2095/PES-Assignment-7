...
size_t tone_to_samples(int freq, uint16_t *buff, size_t size) {
	...
	for (i=0; i < samples; i++) {
		...
		temp1 = sin1(i * TWO_PI /period_in_samples  ) + TRIG_SCALE_FACTOR;
		buff[i]=temp1;

	}

	PRINTF("Generated %d samples at %d Hz; computed period=%d, observed=%d\r\n",
			samples, freq, period_in_samples,
			autocorrelate_detect_period(buff, samples, 0)); //print output as required
